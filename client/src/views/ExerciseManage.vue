  ...
  <div class="page-container">
    <div class="table-card">
      <div class="search-bar">
        <el-input v-model="keyword" placeholder="搜索运动/用户" clearable style="width: 240px" />
      ...
      <el-table :data="tableData" stripe border>
        ...
        <el-table-column prop="exerciseType" label="运动类型" />
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑' : '新增'" width="480px">
      <el-form :model="form" label-width="90px">
        ...
        <el-form-item label="运动类型"><el-input v-model="form.exerciseType" /></el-form-item>
        <el-form-item label="时长(分)"><el-input-number v-model="form.duration" :min="0" /></el-form-item>
        <el-form-item label="消耗热量"><el-input-number v-model="form.caloriesBurned" :min="0" /></el-form-item>
...
async function loadData() {
  const res = await request.get('/exercise/page', { params: { keyword: keyword.value, page: page.value, size: size.value } })
...
async function handleSave() {
  if (form.value.id) await request.put('/exercise', form.value); else await request.post('/exercise', form.value)
  ElMessage.success('保存成功'); dialogVisible.value = false; loadData()
}
async function handleDelete(id) {
  await ElMessageBox.confirm('确定删除？'); await request.delete(`/exercise/${id}`)
